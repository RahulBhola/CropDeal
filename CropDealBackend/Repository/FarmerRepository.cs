...
namespace CropDealBackend.Repository
{
    public class FarmerRepository: IFarmer
    {
        ...
        public async Task<IEnumerable<CropDetails>> GetAllCrops(){
            return await _context.CropDetails.Include(c=>c.User).Where(c => c.User.IsActive).ToListAsync();
        }

        public async Task<CropDetails> GetCropById(int cropId){
            return await _context.CropDetails.FirstOrDefaultAsync(c => c.CropId == cropId);
        }

        public async Task<IEnumerable<CropDetails>> GetCropDetailsByIds(int[] cropIds)
        {
            return await _context.CropDetails
                .Where(c => cropIds.Contains(c.CropId))  // Handle array of IDs
                .ToListAsync();
        }
    }
}