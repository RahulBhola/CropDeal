...
export class ViewCropComponent implements OnInit {
  ...
  onSubmitEdit(): void {
    if(this.editForm.valid && this.selectedCropId){
      ...
      this.farmerService.updateCropDetails(this.selectedCropId, updatedCrop).subscribe({
        next: (response)=>{
          // alert(response.message || 'Crop updated successfully!');
          Swal.fire(
            'Saved!',
        ...
        error: (err)=>{
          ...
          Swal.fire('Error!', 'Failed to update the crop.', 'error');
        }
      });
    }
  }
  ...
  //         Swal.fire('Error!', 'Failed to delete the crop.', 'error');
  //       }
  //     });
  //   }
  // }
  ...
  onDeleteCrop(cropId: number): void {
    Swal.fire({
      title: 'Are you sure?',
      text: 'Do you want to delete this crop? This action cannot be undone.',
      icon: 'warning',
      ...
      if (result.isConfirmed) {
        this.farmerService.deleteCrop(cropId).subscribe({
          ...
          error: (err) => {
            ...
            Swal.fire(
              'Error!',
              'Failed to delete the crop. Please try again later.',
              'error'
            );
          },
        });
